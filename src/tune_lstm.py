...
DATA_PATH = Path(
    "data/processed/"
    "combined_features_AAPL_AMZN_META_NVDA_TSLA_2020-01-01_to_2022-12-31_5d_target_v2_trading_days.csv"
)
SEARCH_RESULTS_PATH = Path("reports/lstm_hyperparameter_search.csv")
BEST_CONFIG_PATH = Path("reports/lstm_best_hyperparameters.json")
TUNED_PREDICTIONS_PATH = Path("reports/lstm_tuned_5d_return_predictions.csv")
TUNED_CHECKPOINT_PATH = Path("model_checkpoints/lstm_tuned_5d_return_best.pt")
...
def build_search_options() -> dict[str, list[object]]:
    return {
        "sequence_length": [5, 10, 15, 20, 30, 45],
        "include_current_row": [True, False],
        "hidden_size": [16, 32, 64, 96],
        "num_layers": [1, 2, 3],
        ...
        "smooth_l1_beta": [0.02, 0.05, 0.1],
        "optimizer_name": ["adam", "adamw"],
        "gradient_clip": [None, 0.5, 1.0],
...
def sample_configs(options: dict[str, list[object]], trials: int, seed: int) -> list[TrialConfig]:
    ...
    while len(sampled) < trials:
        ...
        if hidden_size == 96 and bidirectional and num_layers == 3:
            continue
        if config in seen:
            continue
...
def selection_score(metrics: dict[str, float]) -> float:
    ...
    corr_penalty = 0.0 if np.isnan(corr) else 0.001 * max(0.0, -corr)
    collapse_penalty = max(0.0, 0.002 - metrics["prediction_std"]) * 0.2
    return metrics["rmse"] + 0.25 * metrics["mae"] + corr_penalty + collapse_penalty
