...
\begin{abstract}
\small
Stock price prediction remains challenging due to market noise and non-stationarity. We investigate whether deep learning models enhanced with sentiment analysis can improve next-day price direction prediction. We implement and train LSTM and Transformer architectures on three tech stocks (AAPL, NVDA, TSLA) using technical indicators combined with StockTwits sentiment. After discovering and fixing a critical data leakage issue in our initial experiments, we report corrected results with proper train/validation/test methodology. Key findings: (1) LSTM outperforms Transformer by +5.10\% (52.27\% vs 47.17\%), contrary to initial expectations; (2) data leakage inflated initial results by up to 20 percentage points; (3) severe class imbalance causes prediction collapse; (4) validation-to-test degradation (-6\% to -20\%) indicates poor generalization to 2026 market conditions; (5) Transformer requires more data than available (~415 samples insufficient). Our work demonstrates the critical importance of proper validation methodology and honest reporting of negative results in deep learning research.
\end{abstract}
...
\textbf{A critical lesson learned:} Our initial experiments showed Transformer outperforming LSTM by +4.12\%. However, we discovered a data leakage issue where models were selected based on test set performance. After fixing this and using proper validation, results reversed: LSTM outperforms Transformer by +5.10\%. This paper reports the corrected results and emphasizes the importance of rigorous validation methodology.

\textbf{Key contributions:}
\begin{itemize}
\item Identification and correction of data leakage in model selection
\item Honest comparison showing LSTM outperforms Transformer on limited data
\item Analysis of class imbalance and prediction collapse
\item Demonstration that proper validation can reverse conclusions
\item Evidence that ~415 samples insufficient for Transformer
\end{itemize}
...
We collected daily data for three large-cap technology stocks: AAPL, NVDA, and TSLA. (AMZN excluded due to insufficient data.)
...
\textbf{Total}: ~2,100 stock-days across 3 tickers
...
During code review, we discovered that our initial training script was selecting the best model based on 2026 test set accuracy during training. This is a critical error because:

\begin{enumerate}
\item The model "peeks" at future test data during training
\item Model selection is biased toward test set patterns
\item Reported performance is inflated and unrealistic
\end{enumerate}

\textbf{Impact:} After fixing this issue, our conclusions completely reversed.

\subsection{Experiment 1: LSTM Baseline (Corrected)}

\textbf{Setup:} Train separate LSTM for each ticker using StockTwits sentiment with proper train/val/test split.
...
\midrule
AAPL & 58.65\% & 48.28\% & -10.37\% \\
NVDA & 61.54\% & 41.51\% & -20.03\% \\
TSLA & 58.10\% & 51.72\% & -6.38\% \\
\textbf{Avg} & \textbf{59.43\%} & \textbf{47.17\%} & \textbf{-12.26\%} \\
\bottomrule
\end{tabular}
\caption{LSTM baseline with proper validation. Significant degradation from validation to test indicates poor generalization to 2026 market conditions.}
\end{table}

\textbf{Analysis:} Models perform reasonably on validation set (~59\%) but degrade significantly on 2026 test data (-12.26\% average). NVDA shows worst degradation (-20\%), suggesting market regime change.
